...
class OpenAUCLoss(nn.Module):
    ...
    def loss_open(self, outputs_real, labels, outputs_fake, mask):
        pred_neg, predictions = outputs_real.min(axis=1)
        hit = (predictions.data == labels.data).float() * mask
        pred_pos, _ = outputs_fake.min(axis=1)
        
        return (hit * (pred_neg - pred_pos + 1) ** 2).sum(), hit.sum()
    ...
    def forward(self, logits, labels, f_post, manifolds):
        ...
        loss += loss_fake * self.lambd

        return logits, loss / n if n > 0 else loss